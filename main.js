document.addEventListener('DOMContentLoaded', () => {
    ...
    const planetsBtn = document.getElementById('planetsBtn'); // Added planetsBtn
        ...
        data.forEach(item => {
            ...
            card.innerHTML = `
                ...
                <div class="content">
                    ...
                    ${item.height ? `<p class="underline">Height: ${item.height}</p>` : ''} <!-- Underline added -->
                    ${item.birth_year ? `<p class="underline">Birth Year: ${item.birth_year}</p>` : ''} <!-- Underline added -->
                </div>
            `;
